...
- EPIC-006: Integrate bounded transactional label routing — Completed

| Task     | Description                                                                                                                                                                                                                                                              | Status    | Relevant Files                                                                                                                                                                                                                                                                                                                |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ITEM-017 | Add failing tests for existing-segment placement, owner reroute, multiple foreign reroutes, frozen reservations, preserved anchors, second-pass convergence, resource accounting, and full rollback on failure.                                                          | Completed | `packages/mermaid/src/rendering-util/layout-algorithms/grid/edgeLabels.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/performance.spec.ts`                                                                                                                                                             |
| ITEM-018 | Refactor label placement into at most two transactional passes. Add pass-local label obstacle/projection overlays and route impacted edges once per pass against all frozen reservations.                                                                                | Completed | `packages/mermaid/src/rendering-util/layout-algorithms/grid/edgeLabels.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/router.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerTopology.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerInstrumentation.ts`     |
| ITEM-019 | Replace the existing fixed-cell spatial hash for grid label queries with the same coordinate-compressed interval index where necessary to guarantee no absolute-coordinate-span allocation. Remove the five-order transitional fallback after dense-label parity passes. | Completed | `packages/mermaid/src/rendering-util/layout-algorithms/grid/edgeLabels.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerTopology.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/edgeLabels.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/performance.spec.ts` |

- EPIC-007: Complete migration, performance closure, and legacy removal

| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Status                         | Relevant Files                                                                                                                                                                                                                                                                                                                                                                                 |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ITEM-020 | Audit the resource-cap behavior introduced by ITEM-009 across hierarchy, bundles, and labels. Prove complete-search no-path and label failure never fall back, all fallback routes validate, and post-removal cap failures carry the same fixed reason in `GRID_ROUTE_NOT_FOUND` details.                                                                                                                                                                                               | Completed                      | `packages/mermaid/src/rendering-util/layout-algorithms/grid/router.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/edgeLabels.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerInstrumentation.ts`                                                                                                                                            |
| ITEM-021 | Revisit the deferred EPIC-003 performance debt after hierarchy, bundle, and label routing are implemented; evaluate options against measured topology/search/index bottlenecks, execute the benchmark matrix, and require the existing 1,000/500 case below 500 ms with zero fallback plus all structural/memory caps. Prove canonical minimum-tuple results are byte-identical under queue-order perturbation and `h = 0`; queue discovery order itself is not an acceptance contract. | Completed with target deferred | `packages/mermaid/src/rendering-util/layout-algorithms/grid/performance.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerSearch.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerSearch.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/routerTopology.spec.ts`                                                         |
| ITEM-022 | Run the full grid DDLT corpus with zero grid exemptions, record before/after validity/score/bends/crossings/fallbacks per fixture, review visual diffs, and add exact assertions for intentional route changes. Update `GRID_TOTAL_SCORE_BASELINE` only to the accepted measured result and never below 9470. Verify the swimlane baseline remains exactly 11754 and non-grid tests remain unchanged.                                                                                   | Completed                      | `packages/mermaid/src/rendering-util/layout-algorithms/ddlt/layout-fixtures.ddlt.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/testMatrix.ddlt.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/index.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/edgeLabels.ts`, `e2e/platform/dev-diagrams/layout-tests/grid/*.sizes.json` |
| ITEM-023 | After every removal gate in Section 9 holds, delete `routeWithinContainer()`, corridor selection/helpers, corridor fields from grid metadata, fallback/shadow code, and obsolete tests. Retain sparse-router instrumentation and regression fixtures.                                                                                                                                                                                                                                   | Blocked                        | `packages/mermaid/src/rendering-util/layout-algorithms/grid/router.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/layoutCore.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/types.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/router.spec.ts`, `packages/mermaid/src/rendering-util/layout-algorithms/grid/performance.spec.ts`       |

### EPIC-007 completion evidence

ITEM-020 exercises all four fixed resource-cap reasons, rejects invalid legacy
fallbacks, and adds hierarchy composition coverage in which all three capped
segments validate after `vertex_cap` fallback. Complete-search bundle failure
and two-pass label non-convergence both throw without invoking resource
fallback. The audited cases record zero fallback-validation failures.

ITEM-021 is complete under the explicit project decision to defer, not waive,
the original performance target. The retained expected-failure test measured
the 1,000-node/500-edge case at 1.575 seconds in the final suite. The
instrumented five-run matrix measured a 1493.17 ms median and 1661.13 ms p95,
with 4260 vertices, 16776 directed adjacency entries, 500 endpoint overlays,
844496 expanded states, a 1379-state maximum open set, 1695072 estimated bytes,
and zero fallback. The measured optimization caches vertex points in the two
search arc loops; the controlled compact median changed from 1389.40 ms to
1351.14 ms and the sparse median from 1229.92 ms to 1147.50 ms. NFR-002 and
AC-011 remain unchanged and unmet.

The remaining benchmark matrix was:

| Case                             | Time/result                       | Structural/resource result                                        |
| -------------------------------- | --------------------------------- | ----------------------------------------------------------------- |
| Dense 1,000/500, 48 px gaps      | 1198.29 ms                        | 537174 states, max open 5217, 2690152 bytes, zero fallback        |
| Sparse-coordinate 1,000/500      | 1331.34 ms median, 1862.60 ms p95 | Compact-identical topology/search counts, zero fallback           |
| 10-level nested hierarchy        | 20.23 ms, valid                   | 346 vertices, 1042 arcs, 11 searches, 47 states, zero fallback    |
| 100 forward/reverse pair bundles | 136.98 ms, valid                  | 1212 vertices, 4432 arcs, 200 searches, 600 states, zero fallback |
| Dense measured labels            | 14.49 ms, one pass                | 12 reservations, one reroute, zero full scans, no rollback        |

The synthetic 1,000/500 dense, compact, and sparse layouts still report
`edge-shared-subpath` under whole-layout validation because unrelated synthetic
edges overlap. This is separate from cap completion and is not represented as
a correctness pass. Measured follow-up options are to avoid broad equal-cost
A\* exhaustion, replace repeated predecessor-chain comparisons with indexed
canonical ranks, index committed-route validation, reduce instrumentation's
quadratic shared-segment comparison, and evaluate topology- or target-specific
preprocessing without weakening invocation-local state or canonical tuple
ordering.

ITEM-022 runs 12 non-exempt grid fixtures. Every accepted result is valid, has
zero crossings, zero resource fallback, and zero fallback-validation failures.
The exact aggregate is 9935, above the immutable 9470 floor; the swimlane
aggregate remains 26508 and its `11754` baseline constant is unchanged. All 12
fixtures rendered in headless Chromium with finite view boxes and were visually
reviewed. Browser capture refreshed stale `grid/simple` measurements and the
outdated `routing-group-member` measurements. That second recapture exposed and
fixed a browser-only bundle failure, a renderer-context argument leak, and an
extended label detour whose collinear return leg was normalized away.

| Fixture                         | Legacy valid/score/bends/crossings | Accepted valid/score/bends/crossings | Fallback delta |
| ------------------------------- | ---------------------------------- | ------------------------------------ | -------------- |
| `group-stack`                   | yes / 1000 / 0 / 0                 | yes / 1000 / 0 / 0                   | 0              |
| `placement-matrix-lr`           | yes / 1000 / 0 / 0                 | yes / 1000 / 0 / 0                   | 0              |
| `placement-matrix-tb`           | yes / 1000 / 0 / 0                 | yes / 1000 / 0 / 0                   | 0              |
| `routing-cell-aware-empty-cell` | yes / 965 / 6 / 0                  | yes / 990 / 4 / 0                    | 0              |
| `routing-group-member`          | yes / 485 / 14 / 0                 | yes / 0 / 16 / 0                     | 0              |
| `routing-hierarchy-portals`     | yes / 1000 / 0 / 0                 | yes / 0 / 12 / 0                     | 0              |
| `routing-loops-parallel-lr`     | yes / 985 / 6 / 0                  | yes / 985 / 6 / 0                    | 0              |
| `routing-outside-member`        | yes / 1000 / 0 / 0                 | yes / 970 / 4 / 0                    | 0              |
| `simple`                        | yes / 1000 / 0 / 0                 | yes / 990 / 4 / 0                    | 0              |
| `singleton-alignments`          | yes / 1000 / 0 / 0                 | yes / 1000 / 0 / 0                   | 0              |
| `stack-default`                 | yes / 1000 / 0 / 0                 | yes / 1000 / 0 / 0                   | 0              |
| `stack-gap-zero`                | yes / 1000 / 0 / 0                 | yes / 1000 / 0 / 0                   | 0              |

The legacy column is the EPIC-001 router run against the committed fixture
corpus. `simple` and `routing-group-member` were subsequently recaptured from
the current browser, so their accepted deltas include the measurement migration
...
- 2026-09-22: Completed EPIC-007 ITEM-020 and ITEM-022, completed ITEM-021
  ...
  the legacy router and the generated-case/release-cycle gates are unproven.

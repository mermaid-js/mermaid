...
### Linear Chain Layout Limitations

Long linear flowcharts (chains of nodes in sequence) can become difficult to read when they extend beyond the viewport width. The current layout engine arranges all nodes in a single row or column, which may result in excessive scrolling or cramped diagrams.

#### Current Workarounds

While automatic wrapping is not yet implemented, you can improve readability of long linear chains by:

**1. Using Subgraphs to Group Related Steps**

Break your chain into logical groups using subgraphs:
...
**2. Introducing Branch Points**

Even temporary branches can help redistribute nodes horizontally:
...
**3. Using Multiple Diagrams**

Split a very long sequence into multiple related flowcharts, each with its own clear scope.

#### Future Enhancement

Automatic linear chain wrapping may be supported in future Mermaid releases. Configuration options `wrapLinearChains`, `wrapThreshold`, and `wrapRowSize` are available for future use and do not affect current layouts.

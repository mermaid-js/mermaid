...
- The default `elk.nodePlacementStrategy` is now `NETWORK_SIMPLEX` rather than `BRANDES_KOEPF`. **This changes the layout of existing ELK diagrams**, though most are unaffected: over the ELK edge-case corpus, 9 of 13 diagrams are byte-identical and the rest improve. Set `elk: { nodePlacementStrategy: 'BRANDES_KOEPF' }` to keep the previous placement.

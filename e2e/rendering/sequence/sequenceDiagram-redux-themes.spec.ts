...
test.describe('Sequence Diagram - Redux Themes with Neo Look', () => {
  reduxThemes.forEach((theme) => {
    looks.forEach((look) => {
      test.describe(`Theme: ${theme}, Look: ${look}`, () => {
        test('should render complex sequence with all features', async ({ page }, testInfo) => {
          await imgSnapshotTest(page, testInfo, complexSequenceDiagram, { theme, look });
        });
        ...
        test('should render all participant types', async ({ page }, testInfo) => {
          const diagramCode = `
            sequenceDiagram
              ...
              Col ->> Q: Message 7
          `;
          await imgSnapshotTest(page, testInfo, diagramCode, { theme, look });
        });
        ...
        test('should render with notes and loops', async ({ page }, testInfo) => {
          const diagramCode = `
            sequenceDiagram
              actor Admin
        ...
        test('should render parallel processes', async ({ page }, testInfo) => {
          const diagramCode = `
            sequenceDiagram

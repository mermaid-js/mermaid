...
test.describe('Class diagram — Neo look with new themes', () => {
  // 1. All relationship types + interface + visibility modifiers
  themes.forEach(({ theme, label }) => {
    test(`NEO-1 [${label}]: should render all relationship types with neo look`, async ({
      page,
    }, testInfo) => {
  ...
  themes.forEach(({ theme, label }) => {
    test(`NEO-2 [${label}]: should render cardinality with neo look`, async ({
      page,
    }, testInfo) => {
  ...
  // 3. Generic classes
  themes.forEach(({ theme, label }) => {
    test(`NEO-3 [${label}]: should render generic classes with neo look`, async ({
      page,
    }, testInfo) => {
      await imgSnapshotTest(page, testInfo, diagrams.genericClasses, {
        logLevel: 1,
        htmlLabels: true,
        look: 'neo',
        theme,
  ...
  themes.forEach(({ theme, label }) => {
    test(`NEO-4 [${label}]: should render direction TB diagram with neo look`, async ({
      page,
    }, testInfo) => {
      await imgSnapshotTest(page, testInfo, diagrams.directionTB, {
        logLevel: 1,
        htmlLabels: true,
        look: 'neo',
        theme,
  ...
  themes.forEach(({ theme, label }) => {
    test(`NEO-5 [${label}]: should render full diagram with notes using neo look`, async ({
      page,
    }, testInfo) => {
      await imgSnapshotTest(page, testInfo, diagrams.fullWithNotes, {
        logLevel: 1,
        htmlLabels: true,
        look: 'neo',
        theme,

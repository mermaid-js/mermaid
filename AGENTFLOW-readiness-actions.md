...
**Status:** Proposal — revision 8 (final consistency pass: define connector-designated node, scope `connectorRef` to tools, add `CONNECTOR_REF_NOT_A_CONNECTOR` for the bare-id-but-not-a-connector case)
...
| 8   | Final consistency pass on the connector model: (a) defined **connector-designated node** explicitly in §9.2 — any node carrying one or more of `protocol`/`endpoint`/`transport`/`command`/`auth`/`token_required` qualifies; (b) scoped `connectorRef` to tool definitions only (§9.1, §9.4, §13) — widening to other binding sites is reserved as additive for a later version; (c) defined the third resolution case for bare `connectorRef` — when it resolves to a node that exists but is not connector-designated, emit `CONNECTOR_REF_NOT_A_CONNECTOR` (warn in v0.5.0, error in v1.0); (d) updated Appendix A to mention `connectorRef` rather than `connector`.                                                                                                                                                                                                                                                                                                                                                                                                           |

...
describe('buildSubgraphLayoutOptions', () => {
  it('derives a label-based minimum size for compound nodes', () => {
  ...
  it('leaves layered-specific options off subgraphs without an explicit direction', () => {
    ...
    expect(opts['elk.layered.nodePlacement.bk.fixedAlignment']).toBeUndefined();
  });
  ...
  it('passes through nodePlacementStrategy from config on directional subgraphs', () => {
    const opts = buildSubgraphLayoutOptions(
      { dir: 'TB' },
  ...
  it('defaults nodePlacementAlignment to NONE on directional subgraphs', () => {
    const opts = buildSubgraphLayoutOptions({ dir: 'TB' }, { mergeEdges: true }, 'layered');
    expect(opts['elk.layered.nodePlacement.bk.fixedAlignment']).toBe('NONE');
  });

  it('passes through nodePlacementAlignment from config on directional subgraphs', () => {
    const opts = buildSubgraphLayoutOptions(
      { dir: 'TB' },
  ...
  it('handles undefined elkConfig gracefully', () => {
    const opts = buildSubgraphLayoutOptions({ dir: 'TB' }, undefined, 'layered');
    expect(opts['elk.layered.mergeEdges']).toBeUndefined();
    expect(opts['nodePlacement.strategy']).toBeUndefined();
    expect(opts['elk.layered.nodePlacement.bk.fixedAlignment']).toBe('NONE');
  });

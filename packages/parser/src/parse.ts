...
import type { Info, Packet, Pie, FileTree } from './index.js';

export type DiagramAST = Info | Packet | Pie | FileTree;
...
const initializers = {
  ...
  fileTree: async () => {
    const { createFileTreeServices } = await import('./language/fileTree/index.js');
    const parser = createFileTreeServices().FileTree.parser.LangiumParser;
    parsers.fileTree = parser;
  },
} as const;
...
export async function parse(diagramType: 'fileTree', text: string): Promise<FileTree>;

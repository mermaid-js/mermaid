...
A_Service_Clerk : 01 -- "files" -> W_Purchase_Order id S_File_Order`;
  ...
  it.each([
    ['unpadded single digit', `domainstorytelling-beta\nA_A : 5 -- "works on" -> W_W`, 5],
    ['unpadded one', `domainstorytelling-beta\nA_A : 1 -- "works on" -> W_W`, 1],
    ['zero-padded', `domainstorytelling-beta\nA_A : 01 -- "works on" -> W_W`, 1],
    ['multi digit', `domainstorytelling-beta\nA_A : 12 -- "works on" -> W_W`, 12],
  ])('should accept sequence number: %s', (_name: string, context: string, expected: number) => {
    const result = parse(context);
    expectNoErrorsOrAlternatives(result);
    expect(result.value.sentences[0].noOfSeq).toBe(expected);
  });

  it('should treat a padded and an unpadded sequence number as the same value', () => {
    const padded = parse(`domainstorytelling-beta\nA_A : 07 -- "works on" -> W_W`);
    const bare = parse(`domainstorytelling-beta\nA_A : 7 -- "works on" -> W_W`);
    expectNoErrorsOrAlternatives(padded);
    expectNoErrorsOrAlternatives(bare);
    expect(padded.value.sentences[0].noOfSeq).toBe(bare.value.sentences[0].noOfSeq);
  });

  // 0 would be dropped silently by addSequenceNumberCircles, which skips falsy noOfSeq.
  it.each([
    ['zero', `domainstorytelling-beta\nA_A : 0 -- "works on" -> W_W`],
    ['double zero', `domainstorytelling-beta\nA_A : 00 -- "works on" -> W_W`],
  ])('should reject sequence number: %s', (_name: string, context: string) => {
    const result = parse(context);
    expect(result.parserErrors.length).toBeGreaterThan(0);
  });

...
- [ ] Custom tags/stereotypes support and skin param updates
  ...
  - [x] UpdateElementStyle(elementName, ?bgColor, ?fontColor, ?borderColor, ?shadowing, ?shape, ?sprite, ?techn, ?legendText, ?legendSprite): This call updates the default style of the elements (component, ...) and creates no additional legend entry. It also accepts $showStereotype to override c4.showStereotypes for one element or boundary.
...
A single element or boundary can override the diagram-wide setting with `$showStereotype`, in either direction:

...
### Structurizr-style shapes and styling

The unified renderer styles diagrams to resemble the
[c4model.com](https://c4model.com/diagrams) / Structurizr look: rounded element
boxes with a subtle drop shadow, the element name in bold followed by a muted
`[Stereotype: Technology]` line and a smaller description, dashed relationships
with opaque labels, and light dashed boundaries.

Beyond the default boxes (and the cylinder/queue shapes used for `*Db`/`*Queue`
elements), elements can opt into extra shapes via the `$shape` (on
`UpdateElementStyle`) or `$sprite` attribute:

| Keyword                | Shape                          | Typical use         |
| ---------------------- | ------------------------------ | ------------------- |
| `folder`               | tabbed folder                  | directories         |
| `bucket` / `blob`/`s3` | open-top object-storage bucket | blob/object storage |
| `terminal` / `console` | box with a `>_` prompt         | server-side apps    |
| `browser` / `spa`      | box with a window chrome bar   | single-page apps    |
| `cylinder` / `db`      | cylinder                       | databases           |
| `queue` / `pipe`       | horizontal cylinder            | message queues      |
...
C4Container
    ...
    System_Boundary(c1, "Internet Banking") {
        ...
        ContainerDb(database, "Database", "Oracle 12c", "Stores user info.")
    }
...
Drop shadows are skipped for the hand-drawn look and for elements that set
`$shadowing="false"`. The `$sprite` attribute must be the last argument, after
the description.

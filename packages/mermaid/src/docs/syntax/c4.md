...
[c4model.com](https://c4model.com/diagrams) / Structurizr look: rounded elements
with a light fill and a colored border and matching colored text (outline style,
no shadow), the element name in bold followed by a muted `[Stereotype: Technology]`
line and a smaller description, dashed relationships with clean labels, and light
dashed boundaries.
...
The `$sprite` attribute must be the last argument, after the description.

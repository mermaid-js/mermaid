...
Since v<MERMAID_RELEASE_VERSION>, C4 diagrams wrap by default and can be disabled by either setting wrap or c4.wrap to false. Before v<MERMAID_RELEASE_VERSION>, wrapping was disabled by default, and could be enabled by setting wrap to true (but not c4.wrap).

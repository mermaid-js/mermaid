...
- [ ] Custom tags/stereotypes support and skin param updates
  ...
  - [x] UpdateElementStyle(elementName, ?bgColor, ?fontColor, ?borderColor, ?shadowing, ?shape, ?sprite, ?techn, ?legendText, ?legendSprite): This call updates the default style of the elements (component, ...) and creates no additional legend entry. It also accepts $showStereotype to override c4.showStereotypes for one element.
...
Every element shows its stereotype - the bracketed type line under its name, such as `[Software System]` or `[Container: JavaScript]` - and a boundary shows its type the same way. Set `c4.showStereotypes` to false to leave them all out, which keeps a diagram concise when the shapes already make the types obvious.
...
A single element can override the diagram-wide setting with `$showStereotype`, in either direction:

```
UpdateElementStyle(customerA, $showStereotype="false")
```

This is the equivalent of C4-PlantUML's `HIDE_STEREOTYPE()`, expressed as configuration so that it works the way presentation options do in other Mermaid diagram types.

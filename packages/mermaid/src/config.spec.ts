...
describe('isConfigKeySet', () => {
  beforeEach(() => {
    configApi.reset();
    configApi.setSiteConfig({});
    // `configFromInitialize` deliberately survives `reset`, so clear it too or
    // config left behind by earlier tests in this file leaks in.
    configApi.saveConfigFromInitialize({});
  });

  it('reports a key that only has a default as not set', () => {
    // `layout` always has a value from defaultConfig, which is exactly why the
    // merged config cannot be used to answer this.
    expect(configApi.getConfig().layout).toBeDefined();
    expect(configApi.isConfigKeySet('layout')).toBe(false);
  });

  it('reports a key supplied by a directive as set', () => {
    configApi.addDirective({ layout: 'dagre' });
    expect(configApi.isConfigKeySet('layout')).toBe(true);
  });

  it('reports a key set to the same value as the default as set', () => {
    configApi.addDirective({ layout: configApi.defaultConfig.layout });
    expect(configApi.isConfigKeySet('layout')).toBe(true);
  });

  it('forgets directive-supplied keys after a reset', () => {
    configApi.addDirective({ layout: 'dagre' });
    configApi.reset();
    expect(configApi.isConfigKeySet('layout')).toBe(false);
  });
  ...
  it('does not leak between keys', () => {
    configApi.addDirective({ layout: 'dagre' });
    expect(configApi.isConfigKeySet('theme')).toBe(false);
  });
});

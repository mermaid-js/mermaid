...
const RE_REL_BLOCK = /^(include|extend|generalization|dependency|realization|anchor|constraint|containment|association):/i;
...
const ALLOWED: Record<RelationshipType, [EntityKind, EntityKind][]> = {
  association: [['actor', 'usecase'], ['usecase', 'actor'], ['usecase', 'usecase'], ['usecase', 'collaboration'], ['collaboration', 'usecase'], ['unknown', 'unknown'], ['note', 'unknown'], ['unknown', 'note']],
  include: [['usecase', 'usecase'], ['unknown', 'unknown']],
  extend: [['usecase', 'usecase'], ['unknown', 'unknown']],
  generalization: [['actor', 'actor'], ['usecase', 'usecase'], ['external', 'external'], ['collaboration', 'collaboration'], ['unknown', 'unknown']],
  dependency: [['usecase', 'usecase'], ['usecase', 'external'], ['actor', 'usecase'], ['external', 'usecase'], ['collaboration', 'usecase'], ['unknown', 'unknown']],
  realization: [['usecase', 'usecase'], ['actor', 'usecase'], ['actor', 'external'], ['external', 'usecase'], ['collaboration', 'usecase'], ['unknown', 'unknown']],
  anchor: [['note', 'usecase'], ['note', 'actor'], ['note', 'external'], ['note', 'collaboration'], ['note', 'unknown'], ['unknown', 'note']],
  constraint: [['note', 'note'], ['note', 'usecase'], ['note', 'actor'], ['note', 'external'], ['note', 'collaboration'], ['usecase', 'note'], ['actor', 'note'], ['external', 'note'], ['collaboration', 'note'], ['unknown', 'unknown'], ['unknown', 'note'], ['note', 'unknown']],
  containment: [['usecase', 'usecase'], ['external', 'usecase'], ['collaboration', 'usecase'], ['collaboration', 'note'], ['unknown', 'unknown']],
};
...
function isAllowed(from: string, type: RelationshipType, to: string): boolean {
  ...
  const tk = kindOf(to);
  // Relaxed checking: If one side is unknown, we allow it (the parser will infer it as a usecase later)
  if (fk === 'unknown' || tk === 'unknown') { return true; }
...
export const parseDiagram = (code: string): void => {
  ...
  for (const raw of lines) {
    const line = raw.trim();
    if (!line || line === 'usecaseDiagram' || line.startsWith('%%')) { continue; }

    if (line.startsWith('system')) {
      const m = RE_SYSTEM.exec(line);
      if (m) { setSystem(m[1].replace(/{$/, '').trim()); }
      continue;
    }
    ...
    if (line.startsWith('note ')) {
      ...
      if (noteM) {
        let content = noteM[1].trim();
        if (content.startsWith('"') && content.endsWith('"')) { content = content.slice(1, -1); }
        addNote(noteM[2] || content, content);
      }
      continue;
    }
    ...
    if (relM) {
      ...
      content.split(';').forEach((pair) => {
        ...
        if (pair.includes(sep)) {
          ...
          if (from.trim() && to.trim()) { addConnection(from.trim(), type, to.trim(), lbl.join(':').trim()); }
        }
      });
      continue;
    }

    if (line.startsWith('actor')) { processDefinitions(line.replace(/^actor/, ''), 'actor'); mode = 'actor'; continue; }
    if (line.startsWith('usecase')) { processDefinitions(line.replace(/^usecase/, ''), 'usecase'); mode = 'usecase'; continue; }
    if (line.startsWith('external')) { processDefinitions(line.replace(/^externalSystem|^external/, ''), 'external'); mode = 'external'; continue; }
    if (RE_COLLABORATION.test(line)) { processDefinitions(line.replace(/^collaboration\s+/, ''), 'collaboration'); mode = 'collaboration'; continue; }

    if (line.includes('-->')) {
      const [from, targets] = line.split('-->').map(s => s.trim());
      targets.split(';').forEach(t => {
        const [to, label] = t.split(':').map(s => s.trim());
        if (to) { addConnection(from, 'association', to, label); }
      });
      continue;
    }

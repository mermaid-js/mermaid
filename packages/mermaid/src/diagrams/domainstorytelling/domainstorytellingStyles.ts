...
const getStyles = (options: any) => `
  ...
  /* Different colors for actors vs workobjects. Override via the diagram
     ...
     Every registered theme defines these, so the lineColor fallback is dormant --
     it is here for themes added later, and for third-party ones. Without it an
     undefined variable interpolates as the literal "color: undefined", a
     declaration browsers drop silently. lineColor and not borderColorArray: the
     array separates hues well but is tuned as a border around a filled shape, so
     it only reaches ~2:1 against a light background, while these paint a
     standalone glyph. */
  .domainstorytelling-node-container.actor .domainstorytelling-icon svg {
    color: ${options.domainstorytellingActorColor ?? options.lineColor ?? '#333'};
  }

  .domainstorytelling-node-container.workobject .domainstorytelling-icon svg {
    color: ${options.domainstorytellingWorkobjectColor ?? options.lineColor ?? '#333'};
  }
  ...
  .sequence-number-circle {
    fill: ${options.domainstorytellingSequenceColor ?? options.lineColor ?? '#333'};

...
describe('domainstorytelling parser', () => {
  ...
  describe('sentences and continuations', () => {
    ...
    it('accepts an unpadded sequence number', async () => {
      await parser.parse(buildInput('A_A : 5 -- "works on" -> W_W'));

      expect(db.edges).toEqual([
        { from: 'A_A', to: 'W_W-5', label: 'works on', noOfSeq: 5, sentenceRef: '#5' },
      ]);
    });

    it('treats a padded and an unpadded sequence number as the same value', async () => {
      await parser.parse(buildInput('A_A : 07 -- "works on" -> W_W'));
      const padded = db.edges.map((e) => e.noOfSeq);

      db = new DomainStorytellingDb();
      parser.parser!.yy = db;
      await parser.parse(buildInput('A_A : 7 -- "works on" -> W_W'));

      expect(db.edges.map((e) => e.noOfSeq)).toEqual(padded);
    });

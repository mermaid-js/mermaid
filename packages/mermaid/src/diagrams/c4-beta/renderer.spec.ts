...
${text}`
    ...
    const personStroke = (svg: string) =>
      /\.c4-person rect[^{}]*{\s*stroke:\s*([^;}]+)/.exec(svg)?.[1]?.trim();
    expect(personStroke(defaultSvg)).toBe('#073B6F');
...
`
    ...
    // Theme-resolved swatch: the person identity colour on the outline (stroke).

...
describe('c4-beta db', () => {
  ...
  describe('getData', () => {
    ...
    describe('deployment diagrams', () => {
      ...
      it('should render an infrastructureNode as a leaf box, not a cluster', async () => {
        ...
        // Infrastructure colour now comes from the themed .c4-infrastructureNode CSS rule.
        expect(nodes[0].cssStyles).toEqual([]);
      });
  ...
  it('should apply tags to an element that has children, and to a deployment node', async () => {
    await populate(`c4-beta deployment
style critical stroke:#cc0000

softwareSystem banking "Internet Banking System" :::external {
  container api "API" "Provides an API." "Java"
}
deploymentNode dc "Data Centre" "" "Big Bank plc" :::critical
`);
    const { nodes } = db.getData();

    // a system with children is drawn as a boundary, and its tag still styles that box
    const banking = nodes.find((node) => node.id === 'banking');
    expect(banking?.isGroup).toBe(true);
    expect(banking?.cssClasses).toContain('c4-external');
    expect(banking?.cssClasses).toContain('c4-tag-external');

    // a deployment node is always a boundary, and takes a user tag's inline style
    const dc = nodes.find((node) => node.id === 'dc');
    expect(dc?.cssClasses).toContain('c4-tag-critical');
    expect(dc?.cssStyles).toContain('stroke: #cc0000');
  });
});

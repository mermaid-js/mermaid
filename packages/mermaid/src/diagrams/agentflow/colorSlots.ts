 ...
 */
export function assignColorSlots(nodes: Node[], kindOf: (id: string) => string | undefined): void {
  const palette = (getConfig().themeVariables as { borderColorArray?: unknown })?.borderColorArray;
  const slots = containerSlotCount(Array.isArray(palette) ? palette.length : 0);
  ...
  for (const node of nodes ?? []) {
    ...
    if (node.isGroup || node.shape === 'collapsedGroup') {
      // Wrapped here rather than left to `stampColorSlot`, which takes the modulo against
      // the whole palette and would fold a high container index back onto a kind slot.
      node.colorIndex = KIND_COUNT + (containerOrdinal % slots);
      containerOrdinal++;
      continue;
    }

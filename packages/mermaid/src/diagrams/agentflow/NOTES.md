...
| Thick (`==>`) and dotted-arrow (`-.->`) edge families removed, along with the `thickEdgeText` / `dottedEdgeText` states                     | §5.1 removed those operators. `-.-` (reference) is kept as a single `LINK` token that takes no parameters.                                                                                                                               |

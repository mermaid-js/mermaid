...
export const draw: DrawDefinition = function (txt, id, ver, diagObj) {
  ...
  // function getSwimlaneCount(): number {
  //   return Object.keys(store.swimlanes).length;
  // }
  function addSwimlane(swimlane: Swimlane) {
    store.swimlanes[swimlane.name] = swimlane;
  }
  function fetchSwimlane(key: string): Swimlane | undefined {
    return store.swimlanes[key];
  }
  ...
  function getSwimlane(frame: Frame): Swimlane {
    const key = frame.modelEntityType;
    let swimlaneInStore = fetchSwimlane(key);
    // console.log(`getSwimlane`, {frame,store})
    if (swimlaneInStore === undefined) {
      swimlaneInStore = {
        name: key,
        y: calculateSwimlanePosition(frame) * SWIMLANE_DISTANCE,
      };
      addSwimlane(swimlaneInStore);
    }
    return swimlaneInStore;
  }
  ...
  function renderBox(frame: Frame, swimlane: Swimlane, index: number) {
    diagram
      .append('text')
      .attr('font-weight', 'bold')
      .attr('x', index * 100)
      .attr('y', swimlane.y)
      .attr('fill', calculateEntityColor(frame))
      .text(frame.entityIdentifier);
  }

  interface Context {
    previousFrame?: Frame;
    previousSwimlane?: Swimlane;
  }
  const initial: Context = {};

  ast.frames.reduce((acc, frame, index) => {
    const swimlane = getSwimlane(frame);

    renderBox(frame, swimlane, index);

    acc.previousFrame = frame;
    acc.previousSwimlane = swimlane;
    return acc;
  }, initial);

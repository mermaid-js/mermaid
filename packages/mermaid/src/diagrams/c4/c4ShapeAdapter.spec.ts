...
describe('buildEdgeLabel', () => {
  ...
  it('does not shift the parts up when the relationship has no name', () => {
    // `Rel(a, b, "", "HTTPS")` is accepted. Reading a filtered list back by position printed
    // the technology as the name, and `undefined` as the technology.
    const label = buildEdgeLabel({ label: { text: '' }, techn: { text: 'HTTPS' } }, true);
    expect(label).toBe('<small><i>[HTTPS]</i></small>');
    expect(buildEdgeLabel({ label: { text: '' } }, true)).toBe('');
  });
});

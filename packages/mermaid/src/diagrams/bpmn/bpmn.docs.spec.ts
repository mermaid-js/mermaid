...
const documentationCandidates = [
  resolve(process.cwd(), 'packages/mermaid/src/docs/syntax/bpmn.md'),
  resolve(process.cwd(), 'src/docs/syntax/bpmn.md'),
];
...
const readDocumentation = (): string => {
  ...
  if (!documentationPath) {
    throw new Error(
      `Canonical BPMN documentation was not found at: ${documentationCandidates.join(', ')}`
    );
  }

...
const render = async function (
  ...
  try {
    // Keep upstream's profiling span while still capturing the renderer's return
    // value (agentflow returns its LayoutData, surfaced as `RenderResult.data`).
    const drawResult = injected.profiling

...
export async function adjustLayout(
  ...
  // The title band at the top of each titled subgraph is reserved during layout
  // (see `cluster.ts` → `titleBandHeight`/`calculateGroupBounds`), so the frame
  // already has room for the title and the router has drawn subgraph edges from
  // its true top edge. Paint therefore renders the frames verbatim — no
  // post-layout resizing here (a paint-only grow moved the top up after routing,
  // burying subgraph→external edge starts inside the box).

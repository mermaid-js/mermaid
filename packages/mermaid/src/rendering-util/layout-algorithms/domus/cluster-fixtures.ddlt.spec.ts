...
function loadFixture(name: string): SizesFixture {
  const sizesPath = resolve(fixtureBaseDir(), `${name}.sizes.json`);
  const mmdPath = resolve(fixtureBaseDir(), `${name}.mmd`);
  return loadFreshSizesFixture(sizesPath, mmdPath, name);
}
...
async function runClusterFixture(
  ...
  runOrthogonalEdgePipeline(
    layout,
    withDefaultLibavoidFallback(layout, {
      spacing: 10,

...
function tryLift(pts: Point[], obstacleRects: [string, Rect][], spacing: number): Point[] | null {
  ...
  // Adaptive candidate enumeration (matches obstacleDetourInsertPass pattern).
  // Default spacing alone often fails when a neighbouring obstacle boundary
  // sits between the offender and the candidate.
  const candidates: number[] = isHorizontal
    ? [
        offendingRect.top - spacing,
        offendingRect.bottom + spacing,
        offendingRect.top - spacing / 2,
        offendingRect.bottom + spacing / 2,
        offendingRect.top - spacing * 2,
        offendingRect.bottom + spacing * 2,
      ]
    : [
        offendingRect.left - spacing,
        offendingRect.right + spacing,
        offendingRect.left - spacing / 2,
        offendingRect.right + spacing / 2,
        offendingRect.left - spacing * 2,
        offendingRect.right + spacing * 2,
      ];

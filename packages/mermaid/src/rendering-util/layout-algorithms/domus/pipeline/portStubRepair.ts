...
export function applyPortStubRepairIfNeeded(args: {
  ...
  if (!afterRouting.ok && portMismatchEdgeIds.size > 0) {
    ...
    let endpointStubRepairs = 0;
    if (afterStubs.issues.some((iss) => iss.type === 'edge-bend-near-endpoint')) {
      endpointStubRepairs = repairShortEndpointStubs(data, {
        minLength: Math.max(stubLen, options.spacing ?? 10),
      }).repaired;
    }
    const afterEndpointStubs = endpointStubRepairs > 0 ? validateLayout(data) : afterStubs;
    log.debug(ORTHO_DEBUG, 'ROUTING_GRAPH_PORT_STUBS', {
      edgeCount: portMismatchEdgeIds.size,
      changed,
      stubLen,
      endpointStubRepairs,
      ok: afterEndpointStubs.ok,
      issueCount: afterEndpointStubs.issues.length,
    });
  }
}

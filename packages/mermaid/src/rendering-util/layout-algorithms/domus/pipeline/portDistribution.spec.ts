...
describe('domus/pipeline/portDistribution - ', () => {
  ...
  describe('applyDomusPortDistribution', () => {
    ...
    it('skips self-loops and group endpoints', () => {
      const A: Node = { id: 'A', isGroup: false, x: 0, y: 0, width: 10, height: 10 };
      ...
      const data = { nodes: [A, G], edges } as unknown as LayoutData;
      const nodesById = new Map<string, Node>([
        ['A', A],
        ['G', G],
      ]);
      ...
      // Untouched: self-loop and group-endpoint polylines still have their original centres.
      expect(edges[0].points[0]).toEqual({ x: 0, y: 0 });
      expect(edges[1].points[0]).toEqual({ x: 0, y: 0 });
      expect(edges[1].points[1]).toEqual({ x: 50, y: 50 });
    });

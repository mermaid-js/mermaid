...
export function applySharedSubpathNudge(
  ...
  for (let i = 0; i < vSegs.length; i++) {
    for (let j = i + 1; j < vSegs.length; j++) {
      ...
      const pick = pickNudgeTarget(edges, s1, s2, 'V', opts.preferShorter);
      if (!pick) {
        continue;
      }
  ...
  for (let i = 0; i < hSegs.length; i++) {
    for (let j = i + 1; j < hSegs.length; j++) {
      ...
      const pick = pickNudgeTarget(edges, s1, s2, 'H', opts.preferShorter);
      if (!pick) {
        continue;
      }
      const { target } = pick;
...
function pickNudgeTarget<T extends VSeg | HSeg>(
  ...
  if (preferShorter) {
    ...
    const e1 = edges[s1.edgeIdx] as { label?: unknown };
    const e2 = edges[s2.edgeIdx] as { label?: unknown };
    const e1Labelled = typeof e1.label === 'string' && e1.label.length > 0;
    const e2Labelled = typeof e2.label === 'string' && e2.label.length > 0;
    if (!e1Labelled && e2Labelled) {
      return { target: s1, other: s2 };
    }
    if (e1Labelled && !e2Labelled) {
      return { target: s2, other: s1 };

...
export function applyOcrFallbackIfNeeded(args: {
  ...
  const expansionBudget = targetEdgeIds.size * ocrMaxExpansions;
  const maxFallbackExpansionBudget = 600_000;
  if (expansionBudget > maxFallbackExpansionBudget) {
    log.debug(ORTHO_DEBUG, 'OCR_FALLBACK_SKIPPED_BUDGET', {
      primaryModel,
      targeted: targetEdgeIds.size,
      ocrMaxExpansions,
      expansionBudget,
      maxFallbackExpansionBudget,
    });
    return;
  }

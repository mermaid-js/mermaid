...
function isClear(
  layout: LayoutData,
  edge: { start?: string | number | null; end?: string | number | null },
  points: Point[]
): boolean {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    for (const node of layout.nodes ?? []) {
      ...
      // Group / cluster nodes are containers that hold children, not solid
      // obstacles. Treating their bbox as a wall blocks legitimate repair
      // candidates that need to enter a subgraph to reach a child node
      // (e.g. deploy-pipeline `L_D_F_0` where F lives inside subGraph0).
  ...
  for (const edge of layout.edges ?? []) {
    ...
    if (endNode) {
      ...
      for (const endBandCandidate of endBandCandidates) {
        if (endBandCandidate && isClear(layout, edge, endBandCandidate)) {

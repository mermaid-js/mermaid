...
export function reconcilePortsToLaneOrderRoutingGraph(
  ...
  for (const [key, eps] of byNodeSide.entries()) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // iter-52 fix: only drop the old anchor (pts[1] for start, pts[n-2] for end)
      // when it actually matches the expected stub distance along the port normal.
      // Fallback paths from detourAlignedIfBlocked don't have anchors; dropping
      // pts[1] destroys the detour geometry.
      function isAnchorPoint(p: Point, q: Point, s: PortSide, dist: number): boolean {
        if (s === 'E') {
          return approxEqual(q.x, p.x + dist) && approxEqual(q.y, p.y);
        }
        if (s === 'W') {
          return approxEqual(q.x, p.x - dist) && approxEqual(q.y, p.y);
        }
        if (s === 'S') {
          return approxEqual(q.y, p.y + dist) && approxEqual(q.x, p.x);
        }
        if (s === 'N') {
          return approxEqual(q.y, p.y - dist) && approxEqual(q.x, p.x);
        }
      ...
      if (eps[i].kind === 'start') {
        const hasAnchor = pts.length >= 2 && isAnchorPoint(pts[0], pts[1], side, stubDist);
        if (hasAnchor) {
          // Standard port-anchored path with a stub anchor: replace port & anchor.
          ...
          // Fallback path (no stub anchor): only swap the port, keep the rest.
          (edge as any).points = sanitizeOrthogonalPolylineForRendering([port, ...pts.slice(1)], {
            spacing,
          });
        }
      } else {
        const hasAnchor =
          pts.length >= 2 &&
          isAnchorPoint(pts[pts.length - 1], pts[pts.length - 2], side, stubDist);
        if (hasAnchor) {
          // Standard path: replace port & anchor.
          ...
          // Fallback path: only swap the end port.
          (edge as any).points = sanitizeOrthogonalPolylineForRendering(
            [...pts.slice(0, -1), port],
            { spacing }
          );
        }

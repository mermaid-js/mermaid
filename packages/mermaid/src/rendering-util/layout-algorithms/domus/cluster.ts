...
function calculateGroupBounds(
  ...
  minX -= groupPadding;
  minY -= groupPadding + GROUP_TITLE_PADDING;
  maxX += groupPadding;
  maxY += groupPadding;
...
function signedIntervalSeparation(aMin: number, aMax: number, bMin: number, bMax: number): number {
  if (aMax <= bMin) {
    return bMin - aMax;
  }
  if (bMax <= aMin) {
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      for (let j = i + 1; j < groups.length; j++) {
        ...
        const separationX = signedIntervalSeparation(r1.left, r1.right, r2.left, r2.right);
        const separationY = signedIntervalSeparation(r1.top, r1.bottom, r2.top, r2.bottom);
        const needsHorizontalGap = overlapY > 0 && separationX < minGroupSpacing;
        const needsVerticalGap = overlapX > 0 && separationY < minGroupSpacing;
        if (!needsHorizontalGap && !needsVerticalGap) {
          continue;
        }

        const moveHorizontally = sameLayer
          ? needsHorizontalGap || !needsVerticalGap
          : needsHorizontalGap && !needsVerticalGap;

        if (moveHorizontally) {
          const total = minGroupSpacing - separationX;
          const half = total / 2;
          ...
          const total = minGroupSpacing - separationY;
          const half = total / 2;

/**
 * PARTIALLY-FAILING (migration backlog): 8 of this file's tests call
 * createGraphWithElements() which depends on DOM measurement
 * (getBoundingClientRect) that JSDOM does not implement. The other tests pass.
 *
 * Fix pattern for the failing ones: rewrite to the swimlanes DDLT template
 * (see `company-simp.ddlt.spec.ts` in this folder or
 * `swimlanes/simple-2.ddlt.spec.ts` for the canonical shape).
 */
...
describe('Orthogonal pipeline basic routing', () => {
  ...
  it('keeps self-loop ports visibly separated when the least busy side is too short', () => {
    const A = mkNode('A', 100, 150, 70, 1.5);
    const e1 = mkEdge('e1', 'A', 'A');
...
describe('Orthogonal pipeline path ordering and spacing', () => {
  ...
  it('separates multiple parallel horizontal edges between aligned nodes using spacing', () => {
    const spacing = 10;
    const A = mkNode('A', 100, 150);
    const B = mkNode('B', 300, 150);
    const e1 = mkEdge('e1', 'A', 'B');
  ...
  it('separates multiple parallel vertical edges between aligned nodes using spacing', () => {
    const spacing = 10;
    const A = mkNode('A', 200, 100);
    const B = mkNode('B', 200, 300);
    const e1 = mkEdge('e1', 'A', 'B');
...
describe('Orthogonal pipeline obstacle avoidance', () => {
  ...
  it('avoids running edges through the interior of other nodes in the a-b-c-d cycle scenario after orthogonal node layout', async () => {
    ...
    // Parse into layout data using the flowchart parser.
    await flow.parse(flowchart);
    const layoutData = flow.parser.yy.getData() as LayoutData;

    // Create the graph with DOM elements so node sizes are populated.
...
`;
    ...
    // Populate measured node sizes so DOMUS placement is dimension-aware.
    const svg: Selection<SVGSVGElement, unknown, HTMLElement, any> = select('svg');
    const element = svg.select('g') as unknown as D3Selection<SVGElement>;
    await createGraphWithElements(element, layoutData);

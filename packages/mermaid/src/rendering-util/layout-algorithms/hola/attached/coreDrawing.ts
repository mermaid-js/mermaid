...
import { buildPartLayoutData } from '../parts/parts.js';
import type { DecomposedPart } from '../parts/parts.js';
import type { FlattenResult } from '../core/adapter/flattenFlowchart.js';
import type { DiagnosticCollector } from '../core/diagnostics.js';
import type { Bounds, HolaEdge, HolaGraph, HolaNode, Side } from '../core/model.js';
import { nodeBounds, unionBounds } from '../core/model.js';
import { resolveOptions as resolveHolaOptions } from '../core/options.js';
import { routeFinalEdges } from '../core/routing/finalRouting.js';
import type { FinalEdge, RoutedFinalEdge } from '../core/routing/finalRouting.js';
  ...
  for (const edge of core.edges.values()) {
    ...
    originals.forEach((originalEdgeId, index) => {
      ...
      finalEdges.push({
        ...
        labelClearance: label ? options.labelClearance : undefined,
      });
    });
  }
  for (const loop of flat.selfLoops) {
 ...
 * the sink's bottom face perpendicularly. The mirrored case uses a top exit.
 * This is deliberately a preference, not a hard constraint: the final router
 * keeps its normal route if the outside channel is obstructed or would cross
 * something itself. HOLA's rooted core can orient
...
export function preferOutsideSharedEndpointCorridors(
  ...
  for (const edge of edges) {
    ...
    edge.preferredOutsideSide = outside;
  }
}

 ...
 * the `parts/` stage already applies to its two candidates, with one measure added.
 *
 ...
 *
 * This is the `attached/` stage's own choice; `parts/` keeps drawing its cores
 * exactly as it did.
 */
...
export function drawBestCore(
  ...
  compactGridAxes(layoutData, options);
 ...
 * row/column alignment grid-like chose; it merely gives each axis its own cell
 * size. The flow axis uses rank spacing while its cross axis uses node spacing.
 * Core edges are routed after this step, so their clearance and ports are still
 * calculated from the final node boxes. Before routing, a short ordered sweep
 * restores the appropriate gap wherever two nodes share the other axis; this is
 * what keeps a final convergence node from collapsing into its parent.
 *
 * The 120px floor is grid-like's own default spacing. The transform is deliberately
 * reserved for cells that are more than twice as tall/wide as that axis needs:
 ...
 */
function compactGridAxes(layoutData: LayoutData, options: GridAttachedOptions): void {
  ...
  compactGridAxis(nodes, flowAxis, options.rankSpacing, options.gridSpacing);
  // Containment keeps a group core's rows and columns independent. Its wide labels
  // can therefore inflate the perpendicular square-grid cell without buying any
  // clearance there. Compact that axis too, but leave ordinary attached cores on
  // their established geometry: their tree face search intentionally scores the
  // original cross-axis room when choosing and enlarging attachments.
  if (options.modelCoreGroups) {
    compactGridAxis(
      nodes,
      flowAxis === X_AXIS ? Y_AXIS : X_AXIS,
      options.nodeSpacing,
      options.gridSpacing
    );
  }
...
/** Compact one square-grid axis to the size its node dimension actually needs. */
function compactGridAxis(
  ...
  const axisExtent = Math.max(
    ...nodes.map((node) => (axis === X_AXIS ? (node.width ?? 0) : (node.height ?? 0)))
  );
  const targetStep = Math.max(120, axisExtent + spacing);
  if (targetStep + EPSILON >= gridSpacing) {
    return;
  }

  const scale = targetStep / gridSpacing;
  if (scale > 0.5) {
    return;
  }
  const coordinate = (node: (typeof nodes)[number]) =>
    axis === X_AXIS ? (node.x ?? 0) : (node.y ?? 0);
  ...
  restoreAxisClearance(nodes, axis, spacing);
}

/**
 * Restore the configured clearance after affine compaction where it is visible.
 *
...
function restoreAxisClearance(
  ...
  const axisExtent = (node: (typeof nodes)[number]) =>
    axis === X_AXIS ? (node.width ?? 0) : (node.height ?? 0);
  const crossCoordinate = (node: (typeof nodes)[number]) =>
    axis === X_AXIS ? (node.y ?? 0) : (node.x ?? 0);
  const crossExtent = (node: (typeof nodes)[number]) =>
    axis === X_AXIS ? (node.height ?? 0) : (node.width ?? 0);
    ...
    for (let previousRankIndex = 0; previousRankIndex < rankIndex; previousRankIndex++) {
      ...
      for (const node of rank) {
        for (const previous of previousRank) {
          ...
          minimum = Math.max(
            minimum,
            coordinate(previous) + axisExtent(previous) / 2 + spacing + axisExtent(node) / 2
          );
        }
      }
    }

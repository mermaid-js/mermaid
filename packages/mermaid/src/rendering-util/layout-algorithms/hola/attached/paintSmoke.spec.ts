...
describe('grid-attached paint path', () => {
  ...
  it('marks every route as carrying its own endpoints, so the painter re-clips none', async () => {
    ...
    //
    // `rounded` is not smoothing: it keeps every vertex of the polyline and only
    // replaces the sharp 90° turn at each one with an in-corridor arc, so the
    // route still avoids exactly the obstacles it was routed around. A spline
    // curve (`basis`, `cardinal`, …) would not, which is what this guards.
    for (const edge of layoutData.edges) {
      expect(edge.hasIntersectionPoints, `${edge.id} would be re-clipped`).toBe(true);
      expect(edge.curve, `${edge.id} would be smoothed into a spline`).toBe('rounded');
      expect(edge.roundedCornerRadius, `${edge.id} has no corner radius`).toBeGreaterThan(0);
    }
  });

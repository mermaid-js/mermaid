...
import { collectSubgraphs, fitSubgraphFrames } from './subgraphs.js';
...
function boxesOverlap(first: Box, second: Box): boolean {
  const eps = 1e-6;
  return (
    Math.min(first.maxX, second.maxX) - Math.max(first.minX, second.minX) > eps &&
    Math.min(first.maxY, second.maxY) - Math.max(first.minY, second.minY) > eps
  );
}
...
describe('grid-attached subgraphs', () => {
  ...
  describe('containment', () => {
    ...
    it('does not let sibling frame padding overlap', () => {
      const first = node('first', 'one');
      const second = node('second', 'two');
      first.x = 0;
      first.y = 0;
      second.x = 100;
      second.y = 0;
      const data = layoutData([group('one'), group('two'), first, second], []);
      const frames = fitSubgraphFrames(
        collectSubgraphs(data),
        [first, second],
        resolveGridAttachedOptions(data)
      );
      const one = frames.find((frame) => frame.id === 'one')!.bounds;
      const two = frames.find((frame) => frame.id === 'two')!.bounds;

      // The leaves have a 20px corridor between them. Their sibling borders may
      // use that corridor, but must not paint over one another.
      expect(boxesOverlap(one, two)).toBe(false);
      expect(two.minX - one.maxX).toBeGreaterThanOrEqual(8 - 1e-6);
      expect(contains(one, boxOf(data, 'first'))).toBe(true);
      expect(contains(two, boxOf(data, 'second'))).toBe(true);
    });
  });

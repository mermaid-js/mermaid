...
import { rectsOverlap } from '../core/model.js';
...
function segmentHitsBox(segment: RouteSegment, box: Bounds): boolean {
  const horizontal = Math.abs(segment.a.y - segment.b.y) < 1e-6;
  const vertical = Math.abs(segment.a.x - segment.b.x) < 1e-6;
  if (horizontal) {
    return (
      segment.a.y > box.minY &&
      segment.a.y < box.maxY &&
      Math.max(Math.min(segment.a.x, segment.b.x), box.minX) <
        Math.min(Math.max(segment.a.x, segment.b.x), box.maxX)
    );
  }
  if (vertical) {
    return (
      segment.a.x > box.minX &&
      segment.a.x < box.maxX &&
      Math.max(Math.min(segment.a.y, segment.b.y), box.minY) <
        Math.min(Math.max(segment.a.y, segment.b.y), box.maxY)
    );
  }

  // The final router emits axis-aligned routes. Keep a conservative fallback for
  // malformed input so a diagonal segment is never silently treated as harmless.

/**
 * HOLA — Human-like Orthogonal Layout, exposed as `layout: 'hola'`.
 *
 ...
 *   - `parts/`  The decomposition taken to its diagnostic end — every part
 *               drawn on its own by `grid/` and packed beside the others.
 *   - `attached/` The decomposition put back together: a `grid/` core with each
 *               peeled tree hung back on the node it came from, in the place
 *               `core/`'s face search chooses for it.
 *
 * This file is the last step on top of `attached/`: synthetic subgraph
 * connectivity.
 *
 * The `attached/` stage draws subgraph frames after it has laid out the leaf topology.
...
/**
 * Run the `attached/` pipeline with synthetic edges present, then
...
export function runGridAttachedSubgraphsLayoutCore(data: LayoutData): GridAttachedResult {
  ...
  try {
    ...
    // subgraph-free diagrams keep their compact tree ranks unchanged.
...
export const render = createCommonLayoutRenderer({
  ...
  // module's own router. Preserve them when this variant paints rounded bends:

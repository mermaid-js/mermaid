 ...
 *
 * `layout-tests/hola/` holds the captured corpus. Several of its fixtures have
 * multiple trees hanging off one core, which is exactly what tree placement,
 * rank alignment and slide retraction are for, so the whole corpus is run
 * through the `core/` stage on its own and held to the structural invariants of
 * guide §23.
 */
...
describe('faithful HOLA — wider fixture corpus', () => {
  ...
  const fixtures = discoverLayoutTestFixtures().filter((fx) => fx.profile === 'flowchart-hola');
  ...
  it('places a tree in the corner when every side of its root is taken', async () => {
    const fx = fixtures.find((f) => f.id.includes('graph 5'))!;
    const data = await parseMmdFileToLayoutData(fx.mmdPath, {
      stampFlowchartRendererFields: true,
    });
    applyFixtureContentSizesStrict(data, fx.sizes);

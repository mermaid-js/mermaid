 ...
 *
 * `node.dataDirection` adds the input or output arrow the notation puts in the top-left
 * corner, and `node.isCollection` adds the three-bar collection marker.
 */
export async function bpmnDataObject<T extends SVGGraphicsElement>(
  parent: D3Selection<T>,
  node: Node
): Promise<D3Selection<SVGGElement>> {
  const dataNode = node as BpmnNode;
  ...
  const direction = dataNode.dataDirection;
  if (direction) {
    body
      .append('path')
